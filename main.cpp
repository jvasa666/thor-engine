...
bool value_equal(const Value& a, const Value& b) {
    ...
    if (auto p = std::get_if<Value::ListPtr>(&a.data)) {
        const auto& la = *p; const auto& lb = std::get<Value::ListPtr>(b.data);
    ...
    if (auto p = std::get_if<Value::MapPtr>(&a.data)) {
        const auto& ma = *p; const auto& mb = std::get<Value::MapPtr>(b.data);
...
struct SourceLocation { std::size_t line = 0; std::size_t column = 1; };

enum class OpKind { Call, LoopStart, LoopEnd, IfStart, Else, IfEnd, Break, Continue };
...
struct Instruction {
    ...
    std::string loop_var, loop_count_expr, condition_expr;
    std::optional<std::size_t> jump, alt;
};

struct Program {
    std::vector<Instruction> instructions;
    std::size_t step_limit = 1000000;
};
...
public:
    ...
    Value get_path(const std::string& d) const { return resolve_(d, false); }
    Value require_path(const std::string& d) const { return resolve_(d, true); }
...
private:
    static Value default_for(std::type_index t) {
    ...
    static std::pair<std::string, std::string> split_(const std::string& s) {
        auto p = s.find('.');
    ...
    Value resolve_(const std::string& d, bool strict) const {
        ...
        if (mit != metrics_.end()) cur = &mit->second;
            ...
            if (vit == variables_.end()) {
                if (strict) throw UnknownMetricError("Unknown metric or variable '" + head + "'");
                return Value{};
            }
            cur = &vit->second;
        }
        ...
        while (!rem.empty()) {
            ...
            if (!cur) {
                if (strict) throw UnknownPathError("Unknown path '" + d + "' at '" + walked + "'");
                return Value{};
            }
            rem = ht.second;
        }
